# Disk Append-Log Validation

This package captures the append-log hardening validation for restored gem5 full-system simulation on the current `single-core` lineage.

Scope:
- benchmark: `guest_disk_append_log_ubench`
- source checkpoint: `snapshot_1`
- prepared checkpoint: `snapshot_2`
- guest target path: `/var/tmp/gem5-append-log/commit.log`
...
- it appends ordered records with sequence numbers and checksums
- it performs periodic `fsync(fd)` while the append stream is active
- it closes and reopens the file midway in append mode
- it `fsync`s the parent directory and calls `sync()`
- it reopens the file and verifies the full ordered contents
...
Why we consider this hardening step implemented:
- the restored gem5 run reached the success loop after a log-style append workload with periodic flushes and exact replay verification
...
Artifacts:
- `guest_disk_append_log_ubench.c`: guest workload source
