...
SSHPASS=qflex sshpass -e scp -O -P 2224 -o StrictHostKeyChecking=no \
  ...
  /home/dev/qflex_git/QPoints/scripts/validation/disk_append_log/guest_disk_append_log_ubench.arm64 \
  qflex@127.0.0.1:/tmp/guest_disk_append_log_ubench.arm64
```

```bash
SSHPASS=qflex sshpass -e ssh -o StrictHostKeyChecking=no \
  -o UserKnownHostsFile=/dev/null -p 2224 qflex@127.0.0.1 \
  'nohup /tmp/guest_disk_append_log_ubench.arm64 /var/tmp/gem5-append-log/commit.log cassandra-commitlogish 20000000 128 8 >/tmp/append_log_ubench.stdout 2>&1 &'
```
...
./run_gem5.sh \
  ...
  --experiment disk_append_log_snapshot_2 \
  --snapshot snapshot_2 \
  --inst 250000000 \
  --cores 1 \
  --timing-ruby \
